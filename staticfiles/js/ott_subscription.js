...
function showOtts(value) {
    ...
    var sky_plan_id = 1;  // Adjust as needed
    var randomNumberElement = document.getElementById("randomNumber"); // Get the client ID element
    var clientId = randomNumberElement ? (randomNumberElement.textContent || randomNumberElement.innerText) : null;
    // Fetch the plans for the selected platform
    // Ensure platformId and clientId are valid before making the request
    if (platformId && clientId) {
        fetch(`/ott_subscription/get-skylink-plans/?platform_id=${platformId}&sky_plan_id=${sky_plan_id}&client_id=${clientId}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(data => {
                skylinkPlansList.innerHTML = data.html; // Insert HTML into the correct container
            })
            .catch(error => {
                console.error("Error fetching plans:", error);
                skylinkPlansList.innerHTML = '<p class="text-danger">Failed to load plans. Please try again.</p>';
            });
    } else {
        skylinkPlansList.innerHTML = '<p class="text-warning">Invalid platform or client ID.</p>'; // Handle missing IDs
    }
}
...
function jioshowOtts(value) {
     // Get the target ID from the data-bs-target attribute
    // var targetId = value.dataset.bsTarget;  // This will be something like "#watcho", "#play_box", etc.
     var platformId =4  // Get platform ID
     var skylinkPlansList = document.querySelector('.jio-hotstar'); // Find the .card-container inside this tab
     skylinkPlansList.innerHTML = '';  // Clear the previous content
